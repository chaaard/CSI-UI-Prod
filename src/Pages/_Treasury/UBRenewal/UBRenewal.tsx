...
const handleSaveCustomer = async (id: number, remarks: string) => {
        ...
        const generateInvoice: AxiosRequestConfig = {
         ...
         try {
          ...
          if (result.data === true) {
            ...
            await fetchUBRenewal(analyticsParam);
          } else {
            setIsSnackbarOpen(true);
  ...
  const fetchUBRenewal = useCallback(async(anaylticsParam: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(selectedDate !== null)
        {
          ...
          await fetchUBRenewal(anaylticsParam);
        }
      } catch (error) {
        // Handle error here
        console.error("Error fetching data:", error);
    ...
    fetchData();
  }, [fetchUBRenewal, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchUBRenewal, selectedDate, successRefresh]);
  ...
  const handleRefreshClick = () => {
    try {
      ...
      .then(async () => {
          ...
          await fetchUBRenewalException(exceptionParam);
      })
      .catch((error) => {
        setIsSnackbarOpen(true);
        setSnackbarSeverity('error');
...
const fetchUBRenewalException = useCallback(async(exceptionParam: IExceptionProps) => {
    try {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchUBRenewalException, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
...
}, [fetchUBRenewalException,fetchUBRenewal,isModalClose,modalOpen,isModalCloseException]);
                ...
                <Box sx={{ width: '100%', display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                  <Pagination
                    ...
                    onChange={(event, value) => {
                      ...
                      fetchUBRenewalException(exceptionParam);
                    }}
                  />
                </Box>
              </Box>
