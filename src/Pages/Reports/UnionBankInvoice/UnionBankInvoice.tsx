...
const getUniqueRemarks = (invoices: IGeneratedInvoice[]): string[] => {
  const remarks = invoices
    ...
    .filter((remark): remark is string => remark !== undefined && remark !== null);
...
const handleRemarkChange = (event: SelectChangeEvent<string>) => {
 ...
 const uniqueRemarks = getUniqueRemarks(generatedInvoice);
  ...
  const filteredInvoices = generatedInvoice.filter(invoice => 
    selectedRemark === 'All' || invoice.Remarks === selectedRemark
  );
          ...
          {roleId.toString() === '1' && (
            ...
            <Grid item xs={11.1} sx={{ paddingTop: '15px' }}>
              <FormControl variant="outlined" sx={{marginBottom: 2 }}>
                <InputLabel id="remark-select-label">Filter by Promos</InputLabel>
                <Select
                  size="small" 
                ...
                >
                  ...
                  {uniqueRemarks.map((remark, index) => (
                    <MenuItem key={index} value={remark}>{remark}</MenuItem>
                  ))}
                </Select>
              </FormControl>
            </Grid>
