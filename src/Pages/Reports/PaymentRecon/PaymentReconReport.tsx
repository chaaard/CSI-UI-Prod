...
const PaymentReconReport = () => {
  ...
  const handleExportPaymentRecon = async () => {
    try {
      if(match.length >= 1)
      {
        ...
        match.forEach((row) => {
          const analyticsDate = row.AnalyticsTransactionDate !== null
          ...
          const portalDate = row.ProofListTransactionDate !== null
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchGrabMartMatch, selectedDateFrom, selectedDateTo, selected, selectedStatus]);
