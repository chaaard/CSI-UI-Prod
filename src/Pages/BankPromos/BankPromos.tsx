...
const BankPromos = () => {
  ...
  const handleUploadClick = () => {
    try {
      ...
      if (selectedFile && selectedDate) {
        ...
        formData.append('customerName', 'BankPromos');
        ...
        .then(async (response) => {
          if(response.data.Item2 === 'Proof list already uploaded!')
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('error');
            setMessage('BankPromos proof list already uploaded');
          }
          ...
          else
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('success');
            setMessage('BankPromos proof list uploaded successfully.');
            ...
            await fetchBankPromosMatch(anaylticsParam);
  ...
  const fetchBankPromosPortal = useCallback(async(portalParams: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  const fetchBankPromosMatch = useCallback(async(anaylticsParam: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(selectedDate !== null)
        {
          ...
          await fetchBankPromosPortal(anaylticsParam);
          await fetchBankPromosMatch(anaylticsParam);
          await fetchBankPromosException(exceptionParam);
        }
      } catch (error) {
    ...
    fetchData();
  }, [fetchBankPromos, fetchBankPromosPortal, fetchBankPromosMatch, fetchBankPromosException, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchBankPromosPortal, fetchBankPromosMatch, selectedDate, success, club, match]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchBankPromosException, fetchBankPromos, fetchBankPromosMatch, selectedDate, successRefresh]);
  ...
  return (
    ...
    >
      <Grid container spacing={1} alignItems="flex-start" direction={'row'}>
        <Grid item>
          <HeaderButtons isSubmitted={isSubmitted} handleOpenSubmit={handleOpenSubmit} handleChangeSearch={handleChangeSearch} handleOpenModal={handleOpenModal} handleOpenRefresh={handleOpenRefresh} customerName='BankPromos' handleChangeDate={handleChangeDate} selectedDate={selectedDate} handleOpenGenInvoice={handleOpenGenInvoice} handleExportExceptions={handleExportExceptions} />  
        </Grid>
        <Grid item xs={12}
          sx={{
              paddingTop: '10px',
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                <Pagination
                  ...
                  onChange={(event, value) => {
                    ...
                    fetchBankPromosException(exceptionParam);
                  }}
                />
              </Box>
            </Box>
