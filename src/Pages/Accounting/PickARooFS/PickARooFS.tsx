...
const AcctPickARooFS = () => {
  ...
  useEffect(() => {
    document.title = "Accounting | Pick A Roo - FS";
  }, []);
  ...
  const fetchPickARooFSPortal = useCallback(
  ...
  const fetchPickARooFSMatch = useCallback(
  ...
  const fetchPickARooFS = useCallback(
    async (anaylticsParam: IAnalyticProps) => {
      try {
        const config: AxiosRequestConfig = {
          method: "POST",
  ...
  useEffect(() => {
    ...
    fetchPickARooFS,
    fetchPickARooFSPortal,
    fetchPickARooFSMatch,
    selectedDateFrom,
    selectedDateTo,
    selected,
    jo,
