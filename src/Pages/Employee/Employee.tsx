...
const Employee = () => {
  ...
  const handleUploadClick = () => {
    try {
      ...
      if (selectedFile && selectedDate) {
        ...
        formData.append('customerName', 'Employee');
        ...
        .then(async (response) => {
          if(response.data.Item2 === 'Proof list already uploaded!')
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('error');
            setMessage('Employee proof list already uploaded');
          }
          ...
          else
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('success');
            setMessage('Employee proof list uploaded successfully.');
            ...
            await fetchEmployeeMatch(anaylticsParam);
  ...
  const fetchEmployeePortal = useCallback(async(portalParams: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  const fetchEmployeeMatch = useCallback(async(anaylticsParam: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(selectedDate !== null)
        {
          ...
          await fetchEmployeePortal(anaylticsParam);
          await fetchEmployeeMatch(anaylticsParam);
          await fetchEmployeeException(exceptionParam);
        }
      } catch (error) {
    ...
    fetchData();
  }, [fetchEmployee, fetchEmployeePortal, fetchEmployeeMatch, fetchEmployeeException, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchEmployeePortal, fetchEmployeeMatch, selectedDate, success, club, match]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchEmployeeException, fetchEmployee, fetchEmployeeMatch, selectedDate, successRefresh]);
  ...
  return (
    ...
    >
      <Grid container spacing={1} alignItems="flex-start" direction={'row'}>
        <Grid item>
          <HeaderButtons isSubmitted={isSubmitted} handleOpenSubmit={handleOpenSubmit} handleChangeSearch={handleChangeSearch} handleOpenModal={handleOpenModal} handleOpenRefresh={handleOpenRefresh} customerName='Employee' handleChangeDate={handleChangeDate} selectedDate={selectedDate} handleOpenGenInvoice={handleOpenGenInvoice} handleExportExceptions={handleExportExceptions} />  
        </Grid>
        <Grid item xs={12}
          sx={{
              paddingTop: '10px',
