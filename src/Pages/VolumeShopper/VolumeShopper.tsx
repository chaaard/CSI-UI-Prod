...
const VolumeShopper = () => {
  ...
  useEffect(() => {
    document.title = 'CSI | VolumeShopper';
  }, []);
  ...
  const handleUploadClick = () => {
    try {
      ...
      if (selectedFile && selectedDate) {
        ...
        formData.append('customerName', 'VolumeShopper');
        ...
        .then(async (response) => {
          if(response.data.Item2 === 'Proof list already uploaded!')
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('error');
            setMessage('VolumeShopper proof list already uploaded');
          }
          ...
          else
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('success');
            setMessage('VolumeShopper proof list uploaded successfully.');
            ...
            await fetchVolumeShopperMatch(anaylticsParam);
  ...
  const fetchVolumeShopperPortal = useCallback(async(portalParams: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  const fetchVolumeShopperMatch = useCallback(async(anaylticsParam: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(selectedDate !== null)
        {
          ...
          await fetchVolumeShopperPortal(anaylticsParam);
          await fetchVolumeShopperMatch(anaylticsParam);
          await fetchVolumeShopperException(exceptionParam);
        }
      } catch (error) {
    ...
    fetchData();
  }, [fetchVolumeShopper, fetchVolumeShopperPortal, fetchVolumeShopperMatch, fetchVolumeShopperException, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchVolumeShopperPortal, fetchVolumeShopperMatch, selectedDate, success, club, match]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchVolumeShopperException, fetchVolumeShopper, fetchVolumeShopperMatch, selectedDate, successRefresh]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [REACT_APP_API_ENDPOINT, club, selectedDate]);
  ...
  return (
    ...
    >
      <Grid container spacing={1} alignItems="flex-start" direction={'row'}>
        <Grid item>
          <HeaderButtons isSubmitted={isSubmitted} handleOpenSubmit={handleOpenSubmit} handleChangeSearch={handleChangeSearch} handleOpenModal={handleOpenModal} handleOpenRefresh={handleOpenRefresh} customerName='VolumeShopper' handleChangeDate={handleChangeDate} selectedDate={selectedDate} handleOpenGenInvoice={handleOpenGenInvoice} handleExportExceptions={handleExportExceptions} />  
        </Grid>
        <Grid item xs={12}
          sx={{
              paddingTop: '10px',
