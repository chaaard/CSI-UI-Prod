...
const Others = () => {
  ...
  useEffect(() => {
    setCustomerCode(
      customerCodesByMerch.map((customer) => customer.CustomerCode)
    );
  ...
  const fetchVolumeShopper = useCallback(
  ...
  }, [, selectedDate, successRefresh, submitted, firstEffectDone]);
  ...
  const fetchVolumeShopperException = useCallback(
    async (exceptionParam: IExceptionProps) => {
      try {
        if (
          exceptionParam.dates[0].length > 0 &&
          exceptionParam.memCode.length > 0
        ) {
          setLoading(true);
                    ...
                    {isManualVisible ? (
                      ...
                      >
                        <Grid item xs={12} sx={{ marginLeft: "10px" }}>
                          ...
                          >
                            {customerCodesByMerch.map(
                              (item: IMerchants, index: number) => (
                                <MenuItem
