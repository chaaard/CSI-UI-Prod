...
const UBPizzaVoucher = () => {
  ...
  const fetchUBPizzaVoucher = useCallback(
  ...
  const fetchUBPizzaVoucherException = useCallback(
    async (exceptionParam: IExceptionProps) => {
      try {
        if (exceptionParam.dates[0].length > 0) {
          ...
          const config: AxiosRequestConfig = {
            method: "POST",
            url: `/Adjustment/GetAdjustmentsAsyncUB`,
            data: exceptionParam,
          };
