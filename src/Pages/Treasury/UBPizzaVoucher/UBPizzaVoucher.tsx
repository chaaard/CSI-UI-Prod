...
const UBPizzaVoucher = () => {
  ...
  useEffect(() => {
    document.title = 'CSI | UB Pizza Voucher';
  }, []);
...
const handleSaveCustomer = async (id: number, remarks: string) => {
        ...
        const generateInvoice: AxiosRequestConfig = {
         ...
         try {
          ...
          if (result.data === true) {
            ...
            await fetchUBPizzaVoucher(analyticsParam);
          } else {
            setIsSnackbarOpen(true);
  ...
  const handleSave = async () => { 
    ...
    var analyticsProp: IAnalyticProps = {
        action: "Manual Add UB Pizza Voucher",
  ...
  const fetchUBPizzaVoucher = useCallback(async(anaylticsParam: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(selectedDate !== null)
        {
          ...
          await fetchUBPizzaVoucher(anaylticsParam);
        }
      } catch (error) {
        // Handle error here
        console.error("Error fetching data:", error);
    ...
    fetchData();
  }, [fetchUBPizzaVoucher, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchUBPizzaVoucher, selectedDate, successRefresh]);
  ...
  const handleRefreshClick = () => {
    try {
      ...
      .then(async () => {
          ...
          await fetchUBPizzaVoucherException(exceptionParam);
      })
      .catch((error) => {
        setIsSnackbarOpen(true);
        setSnackbarSeverity('error');
...
const fetchUBPizzaVoucherException = useCallback(async(exceptionParam: IExceptionProps) => {
    try {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchUBPizzaVoucherException, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
...
}, [fetchUBPizzaVoucherException,fetchUBPizzaVoucher,isModalClose,modalOpen,isModalCloseException]);
                ...
                <Box sx={{ width: '100%', display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                  <Pagination
                    ...
                    onChange={(event, value) => {
                      ...
                      fetchUBPizzaVoucherException(exceptionParam);
                    }}
                  />
                </Box>
              </Box>
