...
const handleSaveCustomer = async (id: number, remarks: string) => {
         ...
         try {
          ...
          if (result.data === true) {
            ...
            const analyticsParam: IAnalyticProps = {
              ...
              userId: Id,
              storeId: [club],
              remarks: "ubrebateissuancecsi",
            };

            await fetchUBRebateIssuance(analyticsParam);
          } else {
            setIsSnackbarOpen(true);
  ...
  const fetchUBRebateIssuance = useCallback(async(anaylticsParam: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(selectedDate !== null)
        {
          ...
          await fetchUBRebateIssuance(anaylticsParam);
        }
      } catch (error) {
        // Handle error here
        console.error("Error fetching data:", error);
    ...
    fetchData();
  }, [fetchUBRebateIssuance, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchUBRebateIssuance, selectedDate, successRefresh]);
  ...
  const handleRefreshClick = () => {
    try {
      ...
      .then(async () => {
          ...
          await fetchUBRebateIssuanceException(exceptionParam);
      })
      .catch((error) => {
        setIsSnackbarOpen(true);
        setSnackbarSeverity('error');
...
const fetchUBRebateIssuanceException = useCallback(async(exceptionParam: IExceptionProps) => {
    try {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchUBRebateIssuanceException, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
...
}, [fetchUBRebateIssuanceException,fetchUBRebateIssuance,isModalClose,modalOpen,isModalCloseException]);
                ...
                <Box sx={{ width: '100%', display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                  <Pagination
                    ...
                    onChange={(event, value) => {
                      ...
                      fetchUBRebateIssuanceException(exceptionParam);
                    }}
                  />
                </Box>
              </Box>
...
export default UBRebateIssuance
