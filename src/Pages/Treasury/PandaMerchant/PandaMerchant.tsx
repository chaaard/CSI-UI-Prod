import {
  ...
  const PandaMerchant = () => {
    ...
    useEffect(() => {
      document.title = "CSI | PandaMerchant";
    }, []);
    ...
    const handleUploadClick = async () => {
      try {
        ...
        if (selectedFile && selectedDate) {
          ...
          formData.append("customerName", "PandaMerchant");
          ...
          await api(config)
            .then(async (response) => {
              if (response.data.Item2 === "Proof list already uploaded!") {
                ...
                setIsSnackbarOpen(true);
                setSnackbarSeverity("error");
                setMessage("PandaMerchant proof list already uploaded");
                ...
                setMessage("PandaMerchant proof list uploaded successfully.");
                ...
                await fetchPandaMerchantMatch(analyticsParam);
                await fetchPandaMerchantException(exceptionParam);
                setSuccess(true);
                setOpen(false);
              }
            })
    ...
    const fetchPandaMerchant = useCallback(async (anaylticsParam: IAnalyticProps) => {
      try {
        setLoading(true);
    ...
    const fetchPandaMerchantPortal = useCallback(
    ...
    const fetchPandaMerchantMatch = useCallback(
    ...
    const fetchPandaMerchantException = useCallback(
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          if (selectedDate !== null) {
            ...
            console.log("fetch");
            await fetchPandaMerchant(anaylticsParam);
            await fetchPandaMerchantPortal(anaylticsParam);
            await fetchPandaMerchantMatch(anaylticsParam);
            await fetchPandaMerchantException(exceptionParam);
          }
        } catch (error) {
      ...
      fetchPandaMerchant,
      fetchPandaMerchantPortal,
      fetchPandaMerchantMatch,
      fetchPandaMerchantException,
      page,
      itemsPerPage,
      searchQuery,
      columnToSort,
              ...
              >
                ...
                <Pagination
                  ...
                  onChange={(event, value) => {
                    ...
                    fetchPandaMerchantException(exceptionParam);
                  }}
                />
              </Box>
            </Box>
