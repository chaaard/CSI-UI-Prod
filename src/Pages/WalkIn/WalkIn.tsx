...
const WalkIn = () => {
  ...
  const handleUploadClick = () => {
    try {
      ...
      if (selectedFile && selectedDate) {
        ...
        formData.append('customerName', 'WalkIn');
        ...
        .then(async (response) => {
          if(response.data.Item2 === 'Proof list already uploaded!')
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('error');
            setMessage('WalkIn proof list already uploaded');
          }
          ...
          else
          {
            ...
            setIsSnackbarOpen(true);
            setSnackbarSeverity('success');
            setMessage('WalkIn proof list uploaded successfully.');
            ...
            await fetchWalkInMatch(anaylticsParam);
  ...
  const fetchWalkInPortal = useCallback(async(portalParams: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  const fetchWalkInMatch = useCallback(async(anaylticsParam: IAnalyticProps) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if(selectedDate !== null)
        {
          ...
          await fetchWalkInPortal(anaylticsParam);
          await fetchWalkInMatch(anaylticsParam);
          await fetchWalkInException(exceptionParam);
        }
      } catch (error) {
    ...
    fetchData();
  }, [fetchWalkIn, fetchWalkInPortal, fetchWalkInMatch, fetchWalkInException, page, itemsPerPage, searchQuery, columnToSort, orderBy, selectedDate, club]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchWalkInPortal, fetchWalkInMatch, selectedDate, success, club, match]);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [fetchWalkInException, fetchWalkIn, fetchWalkInMatch, selectedDate, successRefresh]);
  ...
  return (
    ...
    >
      <Grid container spacing={1} alignItems="flex-start" direction={'row'}>
        <Grid item>
          <HeaderButtons isSubmitted={isSubmitted} handleOpenSubmit={handleOpenSubmit} handleChangeSearch={handleChangeSearch} handleOpenModal={handleOpenModal} handleOpenRefresh={handleOpenRefresh} customerName='WalkIn' handleChangeDate={handleChangeDate} selectedDate={selectedDate} handleOpenGenInvoice={handleOpenGenInvoice} handleExportExceptions={handleExportExceptions} />  
        </Grid>
        <Grid item xs={12}
          sx={{
              paddingTop: '10px',
